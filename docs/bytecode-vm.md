...
| `dt` | seconds since the previous frame — what makes a feedback effect rate-independent. Costs a register, and only the programs that read it pay: the header's `flags` bit 1 says which |
...
`dt` deserves a warning. It had its own name in the language and `t`'s register in the compiler, so it was the absolute show time — and nothing failed. `pow(decay, dt * 60)`, which is how the language documents rate-independent decay, saturated instead, so trails never decayed; on hardware a comet filled the strip with stuck pixels over about a minute. It cost a register to fix, and 250 host tests had nothing to say about it, because the wrongness is only visible across frames on a device.
...
- ~~Should a program **declare `dt`'s register in its header**?~~ **Done, and it cost nothing.** The header already had a `flags` field with one bit used, so `bit1 = reads dt` needed no extra byte and no version bump. A program that never mentions `dt` keeps register 15 as scratch, and the VM writes `dt` there **only** when the flag is set — which is what makes it safe, because a value computed in `once` can live in that register and survive into later frames. The corpus went back to a worst case of 30 registers from 31, and the two-layer effect with a screen blend that had stopped compiling compiles again. The compiler decides by emitting once and looking at what it resolved, rather than scanning the source for the name: emission already resolves every builtin through one place, so it cannot disagree with itself, whereas a separate scan would be a second implementation that drifts silently.
